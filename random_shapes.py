...
def _generate_rectangle_mask(point, image, size,p, random,OorA,lean,angle=0):
    ...
    label = ('rectangle', (p1r,p3r),(p1c,p3c),(location[0],location[1]))
...
def _generate_parallelogram_mask(point, image, size,p, random,OorA,lean,angle=0):
    ...
    location=[(p1r+p3r)/2,(p1c+p3c)/2]
    label = ('parallelogram', (p1r,p3r),(p1c,p3c),(location[0],location[1]))
...
def _generate_trapezium_mask(point, image, size,p, random,OorA,lean,angle=0):
    ...
    location=[(p1r+p3r+ d*math.cos(angle))/2,(p1c+p3c+ d*math.sin(angle))/2]
...
file = open('information.txt','w') #创建一个txt文档，向其中写入信息
n=0
while n < 100:
    ...
    try:
        ...
        file.write('picture:{}  Shape: {}      Angle:{:.2f}    Location:{},{}     Color:{}    Size:{}\n'.format(n,labels[0][0],angle,(L[0],L[1]),(L[2],L[3]),colors[0],size))
        plt.close('all')
file.close()
